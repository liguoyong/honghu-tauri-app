    ...
    <div class="layout-wrapper">
        <el-form :inline="true" :model="ruleForm" size="small" ref="planFormRef" class="filter-container" @submit.prevent>
            <el-form-item label="文档名称" prop="title">
                <el-input v-model="ruleForm.title" placeholder="请输入文档标题" clearable />
            </el-form-item>
        ...
        <div class="btns-container">
            <el-button type="primary" size="small" @click="handelCreateDoc">
        ...
        <el-table :data="tableData" size="small" stripe style="width: 100%" @row-dblclick="handelClickViewDetail">
            ...
            <el-table-column prop="identifier" label="文档标识">
                <template #default="{ row }">
                    <el-link type="primary" :underline="false" @click="handleView(row)">{{ row.identifier }}</el-link>
                </template>
            </el-table-column>
            <el-table-column prop="desc" label="文档描述" />
        ...
        <editDocDialog :dialog="updateDialog" @close="handelCloseEditDialog" />
        <doc-drawer :drawer="drawer" />
    </div>
</template>
...
import { getDocList, getDeleteDoc, getDocDetail } from '@/apis/doc'
...
import { filterHtmlText } from '@/utils/index'
...
const tableData = ref([])
const getList = async () => {
    const response = await getDocList({ ...ruleForm, ...pageParams })
...
const handleClickEdit = async (id: number) => {
    router.push({
        name: 'docEdit', query: { id }
    })
}
const handleView = (row: any) => {
    const { identifier = '' } = row
    const url = `/doc/detail?identifier=${identifier}`
